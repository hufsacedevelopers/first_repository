...
// 🌟 1. TypeScript 타입 명시 (에러 해결의 핵심 열쇠!)
...
export default function CommunityPage() {
  const [posts, setPosts] = useState<PostData[]>(initialPosts); // 타입 명시
  ...
  useEffect(() => {
    ...
    if (savedData) {
      setPosts(JSON.parse(savedData) as PostData[]); // 타입 단언
    }
    setIsLoaded(true);
  }, []);
  ...
  const handleAddComment = () => {
    ...
    setPosts(posts.map(post => {
      if (post.id === selectedPostId) {
        const commentEntry: CommentData = { // 타입 명시
          id: Date.now(),
